...
def download_sales_report(request):
    ...
    start_date = timezone.datetime.strptime(
        start_date_str, "%b. %d, %Y, midnight"
    ).date()
    end_date = timezone.datetime.strptime(
        end_date_str, "%b. %d, %Y, midnight"
...
def customers(request):
    ...
    if request.user.is_admin:
        accounts_with_profiles = (
            Account.objects.filter(is_admin=False)
...
def admin_list(request):
    ...
    if request.user.is_admin:
        p = Paginator(
            Account.objects.filter(is_admin=True).order_by("id"), items_per_page
        )
        page = request.GET.get("page")
...
def edit_product(request, product_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            for image in images:
                pic = Images.objects.create(product=product, image=image)
            form.save()
    ...
    context = {"form": form, "image_form": image_form, "images": images}
...
def refund_payment(request, order_id):
    ...
    if access_token is None:
        return JsonResponse(
            {"error": "Failed to authenticate PayPal client"}, status=401
        )
    ...
    if response.status_code == 201:
        ...
        return JsonResponse(
            {"error": "Failed to refund capture"}, status=response.status_code
        )
...
def cancelled_orders(request):
    items_per_page = 15
    p = Paginator(
        Order.objects.filter(status="Cancelled").order_by("-created_at"), items_per_page
    )
    page = request.GET.get("page")
    data = p.get_page(page)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if order_type == "PERCENT" and discount > 80:
                messages.error(
                    request,
                    "The discount percentage should be less than or equal to 80",
                )
                return redirect("add_product_offer")
            form.save()
...
def edit_product_offer(request, product_offer_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            order_type = form.cleaned_data["offer_type"]
            url = reverse(
                "edit_product_offer", kwargs={"product_offer_id": product_offer_id}
            )
            if discount < 0:
                messages.error(request, "The discount rate should not be negative")
                return redirect(url)
...
def edit_category_offer(request, category_offer_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            order_type = form.cleaned_data["offer_type"]
            url = reverse(
                "edit_category_offer", kwargs={"category_offer_id": category_offer_id}
            )
            if discount < 0:
                messages.error(request, "The discount rate should not be negative")
                return redirect(url)
